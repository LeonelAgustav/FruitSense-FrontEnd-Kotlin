package com.fruitsense.app.ui.screen.dashboard.profile

import android.content.Context
import android.net.Uri
import androidx.lifecycle.ViewModel
import androidx.lifecycle.viewModelScope
import com.fruitsense.app.data.UserPreferences
import com.fruitsense.app.data.model.UserProfile
...
class ProfileViewModel @Inject constructor(
    ...
    init {
        // [FIX] Jangan langsung loadProfile().
        // Tunggu sampai Token benar-benar tersedia di DataStore.
        monitorTokenAndLoadProfile()
    }

    private fun monitorTokenAndLoadProfile() {
        viewModelScope.launch {
            // Ambil tokenFlow, filter hanya jika tidak kosong/null, lalu ambil yang pertama (first)
            // Ini memastikan kita menunggu token tersimpan setelah login.
            userPreferences.tokenFlow
                .filter { !it.isNullOrEmpty() }
                .collectLatest { token ->
                    // Token sudah valid/tersedia, sekarang aman untuk load profil
                    loadProfileInternal()
                }
        }
    }

    // Ubah visibility jadi private atau biarkan public untuk refresh manual (Pull to Refresh)
    fun loadProfile() {
        viewModelScope.launch {
            loadProfileInternal()
        }
    ...
    private suspend fun loadProfileInternal() {
        // Hindari loading ulang jika data sudah sukses (opsional, tergantung kebutuhan)
        // if (_profileState.value is ProfileUiState.Success) return
